 ...
 *
 */
package io.cdm.backend.mysql.nio;

import io.cdm.CDMServer;
import io.cdm.backend.mysql.xa.TxState;
import org.slf4j.Logger; import org.slf4j.LoggerFactory;

import io.cdm.backend.mysql.CharsetUtil;
import io.cdm.backend.mysql.SecurityUtil;
...
import io.cdm.util.exception.UnknownTxIsolationException;
...
public class MySQLConnection extends BackendIOConnection {
	...
	private static long initClientFlags() {
		...
		boolean usingCompress= CDMServer.getInstance().getConfig().getSystem().getUseCompression()==1 ;
		if(usingCompress)
		{
	...
	public MySQLConnection(NetworkChannel channel, boolean fromSlaveDB) {
		...
		this.txIsolation = CDMServer.getInstance().getConfig().getSystem().getTxIsolation();
	}
	...
	private void synAndDoExecute(String xaTxID, RouteResultsetNode rrn,
		...
		boolean strictTxIsolation = CDMServer.getInstance().getConfig().getSystem().isStrictTxIsolation();
